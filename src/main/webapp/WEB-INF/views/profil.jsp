<%@page import="com.gts.expersoft.models.Profile"%>
<%@page import="com.gts.expersoft.utils.ApplicationConstants"%>
<%@page import="java.util.ArrayList"%>
<%@page import="com.gts.expersoft.models.Menus"%>
<%@page import="java.util.List"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
    <%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
    <%@ taglib prefix="form" uri="http://www.springframework.org/tags/form"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta charset="utf-8"/>
	    <meta http-equiv="X-UA-Compatible" content="IE=edge"/>
	    <meta name="viewport" content="width=device-width, initial-scale=1"/>
		<link href="${pageContext.request.contextPath}/resources/css/bootstrap.min.css" rel="stylesheet"/>
		<link href="${pageContext.request.contextPath}/resources/css/style.css" rel="stylesheet"/>
		<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
		<script src="${pageContext.request.contextPath}/resources/js/bootstrap.min.js"></script>
		<script src="${pageContext.request.contextPath}/resources/lib/jquery.ntm/js/jquery.ntm.js"></script>
		<link href="http://www.jqueryscript.net/css/jquerysctipttop.css" rel="stylesheet" type="text/css"/>
		<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/lib/jquery.ntm/themes/default/css/theme.css" />
		
		  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
		  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap-theme.min.css">
		  <link href="${pageContext.request.contextPath}/resources/css/navbar.css" rel="stylesheet">
		  <link href="${pageContext.request.contextPath}/resources/css/style.css" rel="stylesheet">
		  
		  <!-- <link rel="stylesheet" href="resources/dist/css/bootstrap-submenu.min.css"> -->
		
		  <!-- <script src="https://code.jquery.com/jquery-3.2.1.min.js" defer></script> -->
		  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js" defer></script>
		  <script src="${pageContext.request.contextPath}/resources/dist/js/bootstrap-submenu.min.js" defer></script>
		  
</head>
<body>
		<style>
            body {
                padding-top: 50px;
            }
            .navbar-template {
                padding: 40px 15px;
            }

        </style>
	<div class="navbar navbar-default navbar-fixed-top" role="navigation">
            <div class="container">
		<div class="navbar-header">
                    <button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
                        <span class="sr-only">XPERSOFT</span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                    </button>
                    <a class="navbar-brand" href="#">XPERSOFT</a>
                </div>
                <div class="collapse navbar-collapse">
                    <ul class="nav navbar-nav">
                    <%
                    	List<Menus> menus = (List<Menus>)session.getAttribute("userMenus");                    		
                    	
                    	for(int i = 0; i < menus.size(); i++){
                    		Menus m = menus.get(i);
                    		
                    		//menus.remove(i);
                    		
                    		%>
                    		<li><a href="#" class="dropdown-toggle" data-toggle="dropdown">
                    				<%=m.getNom()%>
                    			<b class="caret"></b></a>
                    			<ul class="dropdown-menu">
                    			
                    			<%
                    				List<Menus> submenus = m.getSubmenus();
       								
       								if(submenus.size() > 0){
       									for(Menus m3 : submenus){
       										List<Menus> submenus2 = m3.getSubmenus();
       										%>
       										<%if(submenus2 != null && submenus2.size() > 0){ %>
       										<li><a href="#" class="dropdown-toggle" data-toggle="dropdown">
       											<%=m3.getNom()%><b class="caret"></b>
       										</a>
       										
       											<ul class="dropdown-menu">
       											
       												<% 
       													for(int y = 0; y < submenus2.size(); y++){
       														Menus my = submenus2.get(y);
       														%>
       															<li><a href="${pageContext.request.contextPath}/event?id=<%=my.getId()%>">
       																	<%=my.getNom()%>
       																</a>
       															</li>
       														<%
       														
       													}
       												%>
       											
       											</ul>
       										</li>
       										<%
       										}else{
       											%>
       											<li><a href="${pageContext.request.contextPath}/event?id=<%=m3.getId()%>"><%=m3.getNom()%></a></li>
       										
       											<% }
       										}
       								
       								}
                    			%>
                    			</ul>
                    			
                    			</li> 
                    	<%} %>
                    	
                    </ul>
                    <ul class="nav navbar-nav navbar-right">
                    	<li><a href="${pageContext.request.contextPath}?logout"><span class="glyphicon glyphicon-log-in"></span>&nbsp;&nbsp;Sortir</a></li>
                    </ul>
                    </div>
                    </div>
                </div><!--/.nav-collapse -->
     <div class="container">
		<div class="row" style="padding-top: 10px">
		
			<div class="col-md-2 panel panel-default" style="padding: 10px; margin: 10px;">
				  <ul class="nav nav-pills nav-stacked">
					  <li><a href="${pageContext.request.contextPath}/accessibilite/profile?action=new">Nouveau</a></li>
					  <li><a href="#">Annuler</a></li>
					  <li><a href="#">Enregistrer</a></li>
					  <li><a href="#">Supprimer</a></li>
					  <li><a href="#">Rechercher</a></li>
				      <li><a href="#">Imprimer</a></li>
				  </ul>
			</div>
			<div class="row" style="padding: 10px; margin: 10px">
				<form class="form-group" method="POST" 
				action="${pageContext.request.contextPath}/profile/create">
				<div class="col-md-5 panel panel-default" style="padding: 10px; margin: 10px; " >
						<table style="font-size:small;" class="table">
							<tr>
								<td width="30%">Code</td>
								<td width="70%">
								
								<% if(request.getAttribute("profile") != null){%>
								<input type="text" name="code" class="form-control" id="code" disabled="disabled" 
								placeholder="code" value="<%= ((Profile)request.getAttribute("profile")).getId()%>"/></td>
								<%} else if(request.getAttribute("newProfileId") != null){ %>
								<input type="text" name="code" class="form-control" id="code" disabled="disabled" 
								placeholder="code" value="<%= request.getAttribute("newProfileId")%>"/></td>
								<%} else{%>
								<input type="text" name="code" class="form-control" id="code" disabled="disabled" 
								placeholder="code" value="0"/>
								<%}%>
								</td>
							</tr>
							<tr>
								<td width="30%">Nom</td>
								<td width="70%">
								<% if(request.getAttribute("profile") != null){%>
								<input type="text" name="nom" class="form-control" id="nom" placeholder="nom"
								 value="<%= ((Profile)request.getAttribute("profile")).getNom()%>"/>
								<%} else{%>
									<input type="text" name="nom" class="form-control" id="nom" placeholder="nom"
								 value=""/>
								<%} %>
									
									</td>
							</tr>
						</table>
						
						<input type="submit" value="Enregistrer" class="btn btn-primary">
				</div>
				
				<div class="col-md-4 panel panel-warning" style="padding: 10px; margin: 10px">
					<div class="panel-heading">
						Liste des Profiles
					</div>
					
					<div class="panel-body">
						<table class="table table-fixed" style="font-size:small;" width="100%">
							<thead>
								<tr class="warning" style="display: block;">
							       <th width="30%">Login</th>
							       <th width="50%">Nom</th>
							    </tr>
						    </thead>
						    <tbody style="height: 100px; overflow-y: auto; display: block;">
							    <%List<Profile> list = (List<Profile>)session.getAttribute("profileList"); 
							    
							    	for(int i = 0; i < list.size(); i++){
							    		Profile p = list.get(i);
							    		%>
							    		<tr>
							    			<td width="30%"><%=p.getId() %></td>
							    			<td width="50%"><%=p.getNom() %></td>
							    			<td width="20%">
							    				<a href="${pageContext.request.contextPath}/profile/show?pid=<%= p.getId()%>">
										          <span class="glyphicon glyphicon-edit"></span>
										        </a>
							    			</td>
							    		</tr>
							    		<%
							    	}
							    %>
						    </tbody>
						</table>
					</div>
					
				</div>
				<div class="col-md-7 panel panel-warning" style="padding: 10px; margin: 10px;">
					<table class="table table-fixed" style="font-size:small;"> 
						<thead>
							<tr class="warning" style="display: block;">
						       <th width="25%" height="30px" >Order</th>
						       <th width="25%" height="30px" >Menu Principal</th>
						       <th width="25%" height="30px">Sous-Menu-1</th>
						       <th width="25%" height="30px">Sous-Menu-2</th>
						    </tr>
					    </thead>
					    <tbody style="height: 300px; overflow-y: auto; display: block;">
					    <%
					    if(request.getAttribute("profile") != null){
					    		
					    		List<Menus> listp = ((Profile)request.getAttribute("profile")).getMenuses();
					    		
					    		for(int i = 0; i < listp.size(); i++){
						    		Menus m = listp.get(i);
						    		List<Menus> submenus1 = m.getSubmenus();
						    		
						    		%>
						    		<tr>
						    			<td width="25%" style="float: left;">
						    			<%if(m.isChecked()) {%>
						    			<input type="checkbox" name="menuIds" value="<%=m.getId()%>" checked="checked">
						    			<%}else{ %>
						    				<input type="checkbox" name="menuIds" value="<%=m.getId()%>">
						    			<%} %>
						    			</td>
						    			<td width="25%" style="float: left;">
						    				<%=m.getNom() %>
						    			</td>
						    			<td width="25%" style="float: left;"></td>
						    			<td width="25%" style="float: left;"></td>
						    		</tr>
						    		<% 
						    		if(submenus1 != null && submenus1.size() > 0){
						    			for(int j = 0; j < submenus1.size(); j++){
						    				Menus m2 = submenus1.get(j);
						    				List<Menus> submenus2 = m2.getSubmenus();
						    			
						    			%>
						    			<tr>
							    			<td width="25%" style="float: left;">
											<%if(m2.isChecked()) {%>
						    			<input type="checkbox" name="menuIds" value="<%=m2.getId()%>" checked="checked">
						    			<%}else{ %>
						    				<input type="checkbox" name="menuIds" value="<%=m2.getId()%>">
						    			<%} %>							    			</td>
							    			<td width="25%" style="float: left;"></td>
							    			<td width="25%" style="float: left;">
							    				<%=m2.getNom() %>
							    			</td>
							    			<td width="25%" style="float: left;"></td>
							    		</tr>
						    			<%
						    			if(submenus2 != null && submenus2.size() > 0){
						    				for(int y = 0; y < submenus2.size(); y++){
						    					Menus m3 = submenus2.get(y);
						    					%>
						    					<tr>
									    			<td width="25%" style="float: left;">
										<%if(m3.isChecked()) {%>
						    			<input type="checkbox" name="menuIds" value="<%=m3.getId()%>" checked="checked">
						    			<%}else{ %>
						    				<input type="checkbox" name="menuIds" value="<%=m3.getId()%>">
						    			<%} %>									    			</td>
									    			<td width="25%" style="float: left;"></td>
									    			<td width="25%" style="float: left;"></td>
									    			<td width="25%" style="float: left;">
									    				<%=m3.getNom() %>
									    			</td>
									    		</tr>
						    					<%
						    				 }
						    			    }
						    			  }
						    			}
						    		}
					    	}else if(request.getAttribute("action") != null && 
								    request.getAttribute("action").equals(ApplicationConstants.NEW)){
						    	
							    
						    	for(int i = 0; i < menus.size(); i++){
						    		Menus m = menus.get(i);
						    		List<Menus> submenus1 = m.getSubmenus();
						    		
						    		%>
						    		<tr>
						    			<td width="25%" style="float: left;">
						    				<input type="checkbox" name="menuIds" value="<%=m.getId()%>" checked="checked">
						    			</td>
						    			<td width="25%" style="float: left;">
						    				<%=m.getNom() %>
						    			</td>
						    			<td width="25%" style="float: left;"></td>
						    			<td width="25%" style="float: left;"></td>
						    		</tr>
						    		<% 
						    		if(submenus1 != null && submenus1.size() > 0){
						    			for(int j = 0; j < submenus1.size(); j++){
						    				Menus m2 = submenus1.get(j);
						    				List<Menus> submenus2 = m2.getSubmenus();
						    			
						    			%>
						    			<tr>
							    			<td width="25%" style="float: left;">
							    				<input type="checkbox" name="menuIds" value="<%=m2.getId()%>" checked="checked">
							    			</td>
							    			<td width="25%" style="float: left;"></td>
							    			<td width="25%" style="float: left;">
							    				<%=m2.getNom() %>
							    			</td>
							    			<td width="25%" style="float: left;"></td>
							    		</tr>
						    			<%
						    			if(submenus2 != null && submenus2.size() > 0){
						    				for(int y = 0; y < submenus2.size(); y++){
						    					Menus m3 = submenus2.get(y);
						    					%>
						    					<tr>
									    			<td width="25%" style="float: left;">
									    				<input type="checkbox" name="menuIds" value="<%=m3.getId()%>" checked="checked">
									    			</td>
									    			<td width="25%" style="float: left;"></td>
									    			<td width="25%" style="float: left;"></td>
									    			<td width="25%" style="float: left;">
									    				<%=m3.getNom() %>
									    			</td>
									    		</tr>
						    					<%
						    				 }
						    			    }
						    			  }
						    			}
						    		}
						    	}
					    %>
					    </tbody>
					</table>
				</div>
				</form>
			</div>
			
		</div>
			  
		</div>
		
		<script src="https://code.jquery.com/jquery-3.1.1.js"
          integrity="sha256-16cdPddA6VdVInumRGo6IbivbERE8p7CQR3HzTBuELA=" crossorigin="anonymous"></script>
          
        <!-- Include all compiled plugins (below), or include individual files as needed -->
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
        <script src="${pageContext.request.contextPath}/resources/js/navbar.js"></script>
</body>
</html>